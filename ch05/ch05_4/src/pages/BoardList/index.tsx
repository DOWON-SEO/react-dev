...
export default function BoardList({ list, onRemoveList, index, onMoveList, ...props }: BoardListProps) {
    ...
    const children = useMemo(() => cards.map((card, index) => <ListCard key={card.uuid} card={card} onRemove={onRemoveCard(card.uuid)} draggableId={card.uuid} index={index} />), [cards, onRemoveCard]);
    return (
